...
class Shop extends Admin
{
    ...
    public function chat(){
        ...
        $chats = model('app\qyshop\model\Chat')->getPaginateGroupByUid($where,$skip,$limit);
        $allChats = model('app\qyshop\model\Chat')->where($where)->distinct('uid');
        $total = count($allChats);
        $chats = $chats[0]->result;
        foreach( $chats as & $chat){
            $consumer = model('app\qyshop\model\Consumer')->findInfo(['id' => $chat->_id->uid],'username');
            $chat->username = $consumer['username'];
        }
        $this->assign('chats',$chats);
        $this->assign('status',$status);
        $this->assign('page',$page);
        $page_url = "/manage/shop/chat?status=$status&page={page}";
    ...
    public function detail(){
        ...
        if(!$uid || strlen($uid) != 24){
            $this->error(lang('Uid Illegal'),url('manage/shop/chat'));
        }
        $consumer = model('app\qyshop\model\Consumer')->findInfo(['id' => $uid],'username');
        $this->assign('uid',$uid);
    ...
    public function getDetail(){
        ...
        try{
            ...
            $where['sid'] = '0';
            $where['uid'] = $uid;
            $chats = model('app\qyshop\model\Chat')->selectInfo($where,'type,status,content,create_time');
            $ret['data'] = array_map(function($x){ unset($x['id']);return $x->toArray();},$chats);
            //更新消息状态为 已读
            $where['status'] = CHAT_STATUS_UNCHECK;
            $where['type'] = CHAT_TYPE_CONSUMER;
            model('app\qyshop\model\Chat')->read($where);
        }catch (\Exception $e){
            $ret['code'] =  $e->getCode() ? $e->getCode() : ERROR_CODE_DEFAULT;
            $ret['msg'] = $e->getMessage();
        }
    ...
    public function reply(){
        ...
        try{
            ...
            if(!model('app\qyshop\model\Chat')->upsert($data,false)){
                exception(model('app\qyshop\model\Chat')->getError(),ERROR_CODE_DATA_ILLEGAL);
            }
            $consumer  = model('app\qyshop\model\Consumer')->findInfo(['id' => $uid],'M_Code');
            //调用app api,添加通知消息
